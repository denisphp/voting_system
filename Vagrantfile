...
#    options['sync'] do |item|
    for item in options['sync'] do
        config.vm.synced_folder item['synced_folder'], item['vagrant_folder'], type: "nfs", mount_options:['nolock,vers=3,udp,noatime,actimeo=1']
    end
  end
